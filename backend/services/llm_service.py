...
# 宏观政策摘要：联网搜索最新政策/监管/市场环境，供右侧面板展示
PROMPT_MACRO_POLICY = """请联网搜索并整理「国内近期与商业、监管、金融、合规、产业政策相关」的最新政策与市场环境信息（如近期一周内的政策文件、监管动态、重要会议精神等）。

要求：
1. 以宏观视角归纳成一份简明摘要，便于企业做风险与合规参考。
2. 包含：政策/监管要点、对企业的可能影响、建议关注方向。
3. 严格以 JSON 格式输出，且只输出 JSON，不要其他文字：
{
  "title": "宏观政策与市场环境摘要（日期范围）",
  "content": "正文内容，可分段落，用换行分隔。每段简明扼要。"
}
content 建议 300-800 字。"""

# 多维度政策与市场新闻：每条单独一条，不按时间跨度总结。多维度搜集（政策、监管、金融、产业、合规、市场环境等）
PROMPT_MACRO_POLICY_NEWS = """请联网搜索国内近期与「政策、监管、金融、产业、合规、市场环境」相关的多条新闻或动态。

要求：
1. 从多维度分别搜集：例如监管政策、金融动态、产业政策、合规要求、市场环境变化等，每个维度可有多条。
2. 每条新闻单独一条，不要合并成一份时间跨度总结。每条包含：标题、简明内容/摘要、来源、所属维度。
3. 至少返回 5 条、最多 20 条。每条内容 80-200 字。
4. 严格只输出 JSON，不要其他文字，格式如下：
{
  "items": [
    { "title": "新闻标题", "content": "该条新闻的简明摘要或要点", "source": "来源名称或网站", "dimension": "政策|监管|金融|产业|合规|市场环境" },
    ...
  ]
}}
dimension 只能是：政策、监管、金融、产业、合规、市场环境 之一。"""
...
def generate_macro_policy_news_items(api_key=None, base_url=None, model=None, enable_web_search=True):
    ...
    for x in items:
        ...
        if dimension not in ('政策', '监管', '金融', '产业', '合规', '市场环境'):
            dimension = '政策'
