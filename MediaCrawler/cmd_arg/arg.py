...
async def parse_cmd(argv: Optional[Sequence[str]] = None):
    ...
    def main(
        ...
        # sys2 调用时强制使用传入的关键词：优先读环境变量（最可靠），否则读文件，避免被 .env/默认“登录”覆盖
        _kw_env = os.environ.get("SYS2_CRAWL_KEYWORD", "").strip()
        if _kw_env:
            keywords = _kw_env
        else:
            _kw_file = os.environ.get("SYS2_CRAWL_KEYWORD_FILE", "").strip()
            if not _kw_file:
                _kw_file = os.path.join(os.getcwd(), ".sys2_crawl_keyword.txt")
            if _kw_file and os.path.isfile(_kw_file):
                try:
