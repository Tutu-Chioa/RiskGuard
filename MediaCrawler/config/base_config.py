...
KEYWORDS = "编程副业,编程兼职"  # 关键词搜索配置，以英文逗号分隔（可由底部 MEDIACRAWLER_KEYWORDS 覆盖）
...
# 当通过本系统内嵌扫码登录时，会话保存在 browser_data，爬取需用 Playwright 复用该目录
import os as _os
if _os.environ.get("MEDIACRAWLER_USE_PLAYWRIGHT") == "1":
    ENABLE_CDP_MODE = False
# 当由 sys2 等外部调用时，通过环境变量传入爬取关键词（企业名），避免本地默认值（如“登录”）覆盖
if _os.environ.get("MEDIACRAWLER_KEYWORDS", "").strip():
    KEYWORDS = _os.environ.get("MEDIACRAWLER_KEYWORDS", "").strip()
