...
// 直接应用代理到所有请求，然后在代理内部处理路径
app.use(createProxyMiddleware({
  filter: function (req, opt) {
    // 只代理以 /api 开头的请求
...
app.listen(PORT, () => {
  console.log('Final test proxy server running on http://localhost:' + PORT);
  console.log('Proxy to backend API at http://localhost:8005');
  console.log('Filter-based proxy that only forwards /api/* requests');
});