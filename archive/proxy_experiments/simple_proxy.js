...
// 静态文件服务
app.use(express.static(path.join(__dirname, 'risk_frontend/public')));

// 处理SPA路由 - 返回index.html用于所有非API请求
app.get(/^(?!\/api).*$/, (req, res) => {
  const indexPath = path.join(__dirname, 'risk_frontend/build', 'index.html');
  
  // 如果构建目录中的index.html存在，则返回它
