...
app.use('/api', createProxyMiddleware({
  ...
  changeOrigin: true,
  // 当Express路由中间件(/api)截取了/api前缀后，我们需要把它加回去
  pathRewrite: {
    '^/(.*)$': '/api/$1'  // 将 /anything 转换为 /api/anything
  },
  onProxyReq: (proxyReq, req, res) => {
    console.log('Proxying request:', req.method, req.url, '->', 'http://localhost:8005' + proxyReq.path);
  },
...
app.listen(PORT, () => {
  console.log('Working proxy server running on http://localhost:' + PORT);
