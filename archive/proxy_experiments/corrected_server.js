...
// 处理API请求 - 手动转发到后端
app.use('/api', (req, res) => {
  console.log('Received API request:', req.method, req.originalUrl);
  
  // 构建目标URL - 由于Express中间件会截取/api部分，我们需要重新添加它
  let targetUrl = 'http://localhost:8005' + req.originalUrl;
  ...
  const options = {
    ...
    headers: {
      ...req.headers,
      'host': 'localhost:8005',  // 修正Host头
      'connection': 'close'       // 避免连接复用问题
    }
  };
  delete options.headers['content-length']; // 让Node.js自动计算
...
app.listen(PORT, () => {
  console.log('Corrected server running on http://localhost:' + PORT);
  console.log('Manual proxy to backend API at http://localhost:8005');
});