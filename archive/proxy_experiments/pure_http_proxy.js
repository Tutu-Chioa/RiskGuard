...
function httpProxy(req, res) {
  ...
  console.log('Proxying request:', req.method, targetPath);
...
app.listen(PORT, () => {
  console.log('Pure HTTP proxy server running on http://localhost:' + PORT);
  console.log('Proxy to backend API at http://' + BACKEND_HOST + ':' + BACKEND_PORT);
  console.log('Using pure Node.js HTTP module, avoiding http-proxy-middleware entirely');
});