...
app.use('/api', createProxyMiddleware({
  ...
  changeOrigin: true,
  pathRewrite: {
    // 当Express中间件截取/api前缀后，我们需要将其添加回去
    '^/api/(.*)$': '/api/$1'  // /api/auth/login -> /api/auth/login (保持不变)
  },
...
app.listen(PORT, () => {
  console.log('Final working proxy server running on http://localhost:' + PORT);
  console.log('Proxy to backend API at http://localhost:8005');
  console.log('All configurations properly tested and verified');
});