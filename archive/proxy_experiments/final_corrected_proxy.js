...
app.use('/api/', createProxyMiddleware({
  ...
  changeOrigin: true,
  // 修复路径重写：将 /api/... 转发到 /api/... （保持路径一致）
  pathRewrite: (path, req) => {
    console.log('Original path:', path);
    // 确保路径以 /api/ 开头，然后直接使用原路径
    if (path.startsWith('/api/')) {
      const newPath = path; // 直接使用原路径，因为后端也需要 /api/ 前缀
...
app.listen(PORT, () => {
  console.log('Final corrected proxy server running on http://localhost:' + PORT);
  console.log('Proxy to backend API at http://localhost:8005');
  console.log('Using correct path rewriting for API endpoints');
});