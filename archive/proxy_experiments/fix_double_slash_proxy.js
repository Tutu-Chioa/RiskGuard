...
// 使用精确的路径匹配和重写，避免双斜杠问题
app.use('/api', createProxyMiddleware({
  target: 'http://localhost:8005',
  changeOrigin: true,
  pathRewrite: function (path, req) {
    // 手动处理路径重写，确保不会有双斜杠
    const rewrittenPath = path.replace(/^\/api/, '/api');
    console.log('Path rewrite - original:', path, '-> rewritten:', rewrittenPath);
...
app.listen(PORT, () => {
  console.log('Fix double slash proxy server running on http://localhost:' + PORT);
  console.log('Proxy to backend API at http://localhost:8005');
  console.log('Corrected path rewriting to avoid double slashes');
});